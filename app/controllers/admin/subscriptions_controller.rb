class Admin::SubscriptionsController < ApplicationController
  ...
  def index
    @subscriptions = Subscription.active.includes(:user).order(created_at: :desc).page(params[:page]).per(10)
  end
  ...
  def create
    @subscription = Subscription.new(subscription_params.merge(plan_scheme: 'yearly'))

    respond_to do |format|
      if @subscription.save && @subscription.start
  ...
  private
    ...
    def subscription_params
      params.require(:subscription).permit(:start_date, :plan_type, :user_id)
    end
end
